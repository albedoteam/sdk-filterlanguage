namespace AlbedoTeam.Sdk.FilterLanguage.Definitions
{
    ...
    /// <summary>
    ///     Represents an operator that has two parameters, one on the left and one on the right
    /// </summary>
    /// <seealso cref="OperatorDefinition" />
    internal class BinaryOperatorDefinition : OperatorDefinition
    {
        ...
        public BinaryOperatorDefinition(string name,
            ...
            : base(name, regex, orderOfPrecedence, LeftRight, param =>
            {
                var left = param[0];
                var right = param[1];
